...
# IMPORTANT: Replace YOUR_GOOGLE_DRIVE_FILE_ID below with your actual file ID
# How to get it: Upload disease_model.h5 to Google Drive → Share (Anyone with link) 
# → Copy the ID from the URL: https://drive.google.com/file/d/THIS_IS_THE_ID/view
...
# Reverse mapping: {0: 'Bacterial leaf blight', 1: 'Brown spot', 2: 'Leaf smut'}
...
def health():
    return jsonify({'status': 'ok', 'disease_model': disease_model is not None})
...
def predict_disease():
    try:
        ...
        medicine = treatment.get('medicine', 'No treatment needed')
        ...
        if is_diseased:
            ...
            recommendations.append({'type': 'success', 'icon': '✅', 'text': 'Crop looks healthy! Continue current care.'})
